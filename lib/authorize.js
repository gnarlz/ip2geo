...
const key = async (key, requestId) => {
  ...
  return redisClientSendCommand('GET', args)
    .catch((error) => {
      logger.log({ requestId, level: 'error', src: 'lib/authorize.key', key: akey, message: 'error', error: error.message })
    ...
    .then((results) => {
      if (results) {
        ...
        if (data.authorized) {
          ...
          logger.log({ requestId, level: 'error', src: 'lib/authorize.key', key: akey, message: 'API key unauthorized in Redis' })
        ...
        logger.log({ requestId, level: 'error', src: 'lib/authorize.key', key: akey, message: 'API key not found in Redis' })
        const error = new Error()
